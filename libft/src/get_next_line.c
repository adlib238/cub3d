...
/*   Updated: 2022/05/23 14:01:39 by kfumiya          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*get_line(char	*save_chr, char	**save)
{
	...
	line = ft_strsub(*save, 0, check_n(*save) + 1);
	if (line == NULL)
		return (NULL);
	tmp = ft_strsub(*save, check_n(*save) + 1, ft_strlen(save_chr + 1));
	if (tmp == NULL)
	{
		brainless_free(&line);
		return (NULL);
